...
csvfile1 = open('node_list.csv', 'r')
csvfile2 = open('edge_list.csv', 'r')
csvfile3 = open('network.csv', 'w')
...
def csv2txt():
	...
	i=0
	for counter, value in enumerate(connected_list, 0):
		if value == 'FALSE':
			continue
		if value == 'TRUE':
			i+=1
			# print(node_list[counter])
			writecsv.writerow(node_list[counter])

	print('Number of nodes: {0}' .format(len(node_list)))
	print('Number of connected nodes: {0}' .format(i))
...
csvfile3 = open('network.csv', 'r')
readcsv3 = csv.reader(csvfile3, delimiter=',')
txtfile = open('network.txt', 'w')
