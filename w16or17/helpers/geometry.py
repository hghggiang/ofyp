...
class point:
	...
	def inside_polygon(self, obstacles):
	    for polygon in obstacles:
		    ...
		    inside = False;
		    p1x, p1y = points[0];
		    for i in range(1, n + 1): # range from 1 to (n+1)
		        p2x, p2y = points[i % n]; # remainder when dividing i by n
		        if y > min(p1y, p2y):
		            if y <= max(p1y, p2y):
		                if x <= max(p1x, p2x):
		                    if p1y != p2y:
...
def PathSmoothing(path, maxIter, obstacles):
	print("PathSmoothing")
	...
	for i in range(maxIter):
		# Sample two points
		pickPoints = [uniform(0,le), uniform(0,le)]
		pickPoints.sort()
		# print(pickPoints)
		...
		first_point = point(first[0], first[1])
		second_point = point(second[0], second[1])
		segment = [first_point,second_point]
        #collision check
		if not check_obstruction(obstacles, segment):
			continue
