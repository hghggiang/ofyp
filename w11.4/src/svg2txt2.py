...
def SVG2TXT(inputFilename, retIndex=False):
    """
    """
    # Generate the description file from SVG
    svg_dom = minidom.parse(inputFilename)

    # Extract nodes
    # Normal nodes
    node_strings_circle = [[path.getAttribute('cx'), path.getAttribute('cy')]
                           for path in svg_dom.getElementsByTagName('circle')]
    terminalNodesStartIndex = len(node_strings_circle)
    # Terminal nodes
    node_strings_rect = [[path.getAttribute('x'), path.getAttribute('y')]
                         for path in svg_dom.getElementsByTagName('rect')]
    all_node_positions = []
    for s in node_strings_circle:
        # The current version of the planner needs the positions to be integers
        all_node_positions.append((Float2Int(float(s[0])), Float2Int(float(s[1]))))
    for s in node_strings_rect:
        # The current version of the planner needs the positions to be integers
        all_node_positions.append((Float2Int(float(s[0])), Float2Int(float(s[1]))))
    ...
    for line in lines:
        ...
        for point in line:
            ...
            min_dist = np.inf
            for i, node in enumerate(all_node_positions):
    ...
    adj = {i: [] for i in xrange(len(all_node_positions))}
    ...
    for node, neighbors in adj.items():
        datastring += linesep
        linesep = '\n'
        datastring += '{0},{1}'.format(all_node_positions[node][0],
                                       all_node_positions[node][1])
        for neighbor in neighbors:
            datastring += sep
            datastring += '{0},{1}'.format(all_node_positions[neighbor][0],
                                           all_node_positions[neighbor][1])

    if retIndex:
        return datastring, terminalNodesStartIndex
    else:
        return datastring
...
if __name__ == "__main__":
    ...
    datastring = SVG2TXT(args.input_filename)
